...
use TgkwAdc\Annotation\SystemPermission;
...
class SystemPermissionHelper
{
    ...
    private static function collectClassAnnotations(): array
    {
        $result = [];
        // 获取类注解映射（类名 => 注解实例）
        $annotations = AnnotationCollector::getClassesByAnnotation(SystemPermission::class);

        foreach ($annotations as $className => $annotation) {
            if (! $annotation instanceof SystemPermission) {
                continue; // 过滤非目标注解
            }
            $result[] = [
                'type' => 'class',
    ...
    private static function collectMethodAnnotations(): array
    {
        ...
        $methodAnnotations = AnnotationCollector::getMethodsByAnnotation(SystemPermission::class);
